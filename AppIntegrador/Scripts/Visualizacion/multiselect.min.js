if (!m_helper) var m_helper =
{
    ...
    {
        return  a || (a = {}),
                a.tag = 'div', this.create(a)
    },
    label: function (a)
    {
        return  a || (a = {}),
                a.tag = 'label', this.create(a)
    },
    textField: function (a)
    {
        return  a || (a = {}), a.tag = 'input',
            a.attributes || (a.attributes = {}),
            a.attributes.type = 'text', this.create(a)
    },
    checkbox: function (a)
    {
        return  a || (a = {}),
...
Multiselect.prototype = {
    ...
    _createItemList: function ()
    {
        ...
        var c = this._createItem('span', -1, 'Seleccionar todo'),
            d = m_helper.div(
                {
                    id: this._getItemListIdentifier(),
                    class: 'multiselect-list'
                });
        return d.appendChild(c), d.appendChild(m_helper.create({ tag: 'hr' })), d.appendChild(a), d
    },
    _createItem: function (a, b, c)
    {
        var f = m_helper.checkbox(
            {
                class: 'multiselect-checkbox', data: { val: b }
            }),
            g = m_helper.create(
                {
                    tag: 'span', class: 'multiselect-text'
                }),
            h = m_helper.create({ tag: a }),
            j = m_helper.label();
    ...
    {
        var c = this;
        a && (m_helper.each(document.getElementById(this._getIdentifier()).querySelectorAll('.multiselect-checkbox'),
            function (d)
            {
    ...
    _appendEvents: function ()
    {
        ...
        document.getElementById(
            a._getInputFieldIdentifier()).addEventListener('focus', function (c)
            {
                a._showList(a),
            ...
            document.getElementById(a._getInputFieldIdentifier()).addEventListener('click',
                function ()
                {
                    a._showList(a), document.getElementById(a._getInputFieldIdentifier()).value = ''
                }),
            document.getElementById(a._getIdentifier()).addEventListener('click',
                function (c)
                {
                    c = c || window.event; var d = c.target || c.srcElement; m_helper.parent(d, a._getIdentifier()) && c.stopPropagation()
                }),
            document.getElementById(a._getItemListIdentifier()).addEventListener('mouseover',
                function ()
                {
                    a._showList(a)
                }),
        ...
        var b = function ()
            {
                ...
                else
                {
                    ...
                    var d = Array.prototype.filter.call(document.querySelectorAll('#' + a._getItemListIdentifier() + ' li span'),
                        function (f)
                        {
    ...
    {
        var c = b._getItems();
        a.checked ? (b._itemCounter = c.length, m_helper.each(c,
            function (d)
            {
            ...
            })) : (b._itemCounter = 0, m_helper.each(c,
                function (d)
                {
    ...
    _hideList: function (a, b)
    {
        ...
        a._updateText(a), b && b.stopPropagation()
    },
    _updateText: function (a)
    {
        ...
        if (0 < b.length)
        {
            for (var c = '', d = 0; d < (5 > b.length ? b.length : 5); d++)
                c += b[d].innerText + ', ';
            c = c.substr(0, c.length - 2), 20 < c.length && (c = c.substr(0, 17) + '...')
        }
        b.length == document.getElementById(a._getItemListIdentifier()).querySelectorAll('ul li').length && (c = 'Todos seleccionados'), document.getElementById(a._getInputFieldIdentifier()).value = c ? c : ''
    },
    ...
    _forceUpdate: function ()
    {
        ...
        if (a.style.visibility = 'hidden', 0 != this._itemCounter)
        {
            ...
            var b = a.nextElementSibling; 10 > this._itemCounter ? (a.style.left = '-45px', b.style.marginLeft = '-42px') : 100 > this._itemCounter ? (a.style.left = '-50px', b.style.marginLeft = '-47px') : 1e3 > this._itemCounter ? (a.style.left = '-55px', b.style.marginLeft = '-52px') : 1e4 > this._itemCounter && (a.style.left = '-60px', b.style.marginLeft = '-57px')
        }
    },
    _items: void 0, _itemCounter: 0, _getItems: function ()
    {
...
window.multiselects = [],
    'undefined' == typeof $ ? (document.multiselect =
        function (a)
        {
            var b = [];
            return window.multiselects._items || (window.multiselects._items = []),
                m_helper.each(document.querySelectorAll(a),
                    function (c)
                    {
                        var d = window.multiselects._items.indexOf(c);
                        if (-1 == d)
                        {
                            var f = new Multiselect(c); window.multiselects.push(f), window.multiselects._items.push(c), b.push(f)
                        }
                        else
...
/*Este documento ha sido tomado desde https://github.com/mneofit/multiselect con modificaciones para efectos de este proyecto*/