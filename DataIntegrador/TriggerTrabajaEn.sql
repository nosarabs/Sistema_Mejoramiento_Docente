CREATE TRIGGER [TriggerTrabajaEn]
	...
	from inserted i
	BEGIN
		if((@CodUnidad not in (select CodUnidadAcademica from Trabaja_en) or @Correo not in (select CorreoFuncionario from Trabaja_en)) and (@CodUnidad not like '' and @Correo not like ''))
		begin
